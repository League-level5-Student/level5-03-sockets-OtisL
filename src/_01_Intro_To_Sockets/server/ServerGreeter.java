	...
	public void run() {
		...
		while(stuff) {
			...
			try {
				...
				System.out.println("Timeout in 20 seconds");
			...
			//7. If the program catches a IOException, let the user know about it and set the loop's boolean variable to false.
				System.err.println("An IOException has been thrown from the server. The server will now be terminated.");
				stuff=false;
			}
		}
	...
	public static void main(String[] args) {
		...
		Thread t = new Thread(()->{
			try {
				...
				System.err.println("An IOException has been thrown while creating the server.");
			}
		});
		t.start();
	}
